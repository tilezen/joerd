...
class OTRTile(object):
	...
	def freeze_dry(self):
		return dict(type='otr', link=self.link)

IS_OTR_FILE = re.compile(
...
def _parse_otr_tile(link, parent):
	m = IS_OTR_FILE.match(link)
	...
	return OTRTile(parent, link, name, block, bbox)

class OTR(object):
	def __init__(self, options={}):
		self.base_dir = options.get('base_dir', 'otr')
	...
	def _ensure_tile_index(self):
		if self.tile_index is None:
			...
			self.tile_index = index.create(index_file, bbox, _parse_otr_tile, self)
	...
	def rehydrate(self, data):
		assert data.get('type') == 'otr', \
			"Unable to rehydrate %r from Slovenia." %data
		return _parse_otr_tile(data['link'], self)
