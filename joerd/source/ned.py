...
NED_IMG_PATTERN = re.compile('^ned19_' \
                             '([ns])([0-9]{2})x([0257][05])_' \
                             '([ew])([0-9]{3})x([0257][05])_' \
                             '[a-z]{2}_[a-z]+_topobathy_20[0-9]{2}.(zip|img)$')


def _ned_parse_filename(fname):
    m = NED_IMG_PATTERN.match(fname)
...
class NED:
    ...
    def buildvrt(self):
        ...
        for f in glob.glob(os.path.join(self.base_dir, '*.img')):
            bbox = _ned_parse_filename(os.path.split(f)[1])
